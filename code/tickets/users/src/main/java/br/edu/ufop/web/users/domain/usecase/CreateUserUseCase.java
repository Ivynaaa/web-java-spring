...
public class CreateUserUseCase {
    ...
    public void validate() {
        //validações conforme a regra de negocios para a criação do usuario
        validateName();
    ...
    private void validateCCNetworkId() {
        if(this.userDomain.getCcNetwork() == null){
            throw new UseCaseException("Invalide credit car");
        }
        UUID id = this.userDomain.getCcNetwork().getId();

        if(id == null){
            throw new UseCaseException("Credit card Id is null");
        }

        Optional<CCNetworkDomain> domainOptional = repositoryPort.findById(id);

        if(domainOptional.isEmpty()){
            throw new UseCaseException("Credit card does not exist");
        }
